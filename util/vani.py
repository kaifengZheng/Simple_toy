...
class DAR_data_Vani(pl.LightningDataModule):
    def __init__(
            self,
    ...
    def prepare_data_train(self):
        ...
        y_data_s = pd.read_csv(self.data_dir + "/" + self.y_data_s_path,index_col=0)
        if self.normal_x:
        ...
        data=[]
        indices=self.discriptor
        keys=list(x_data_s.keys())   
        labels=[]
        input=[]
        for i in range(len(keys)):
            temp_label=[]
            input.append(np.array(x_data_s[keys[i]]))
            for j in range(len(indices)):
                temp_label.append(np.float32(y_data_s[keys[i]].loc[indices[j]]))
            labels.append(
                np.array(temp_label)
            )
        for i in range(len(keys)):    
            data.append([np.array(x_data_s[keys[i]]),np.array(labels[i])])
        # print(len(data))
        return data
    ...
    def prepare_predict_data(self,
        ...
        for i in range(len(keys)):
            data_x.append(np.array(x_data_s[keys[i]]))
        return tourch.from_numpy(data_x.astype(np.float32))
...
def data_generator(data, train:bool, batch_size):
    ...
    while True:
        if train == True:
            x_batch_s,y_batch = generate_examples(batch_size, data)
        else:
        ...
        yield torch.from_numpy(x_batch_s.astype(np.float32)),torch.from_numpy(
...
def generate_examples(num_examples, data):
    ...
    return x_s_examples, label_examples_s
...
class NN_model_vani(pl.LightningModule):
    def __init__(self,net:torch.nn.Module,loss_func,lr=1e-1):
        super().__init__()
        self.net=net
        self.loss=loss_func
    ...
    def training_step(self,batch,batch_idx):
        ...
        for name, param in self.named_parameters():
            if param.grad is not None:
                self.log(f"gradients/{name}", param.grad, on_step=True, on_epoch=True)
        return loss
